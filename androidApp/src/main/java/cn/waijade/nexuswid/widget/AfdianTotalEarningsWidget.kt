...
class AfdianTotalEarningsWidget : GlanceAppWidget() {
    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        provideContent {
            AfdianTotalEarningsContent(earnings, errorMsg, isDark, prefs.isConfigured)
        }
    }
    ...
    companion object {
        private const val TAG = "AfdianTotalWidget"

        suspend fun updateAll(context: Context) {
            val manager = GlanceAppWidgetManager(context)
            manager.getGlanceIds(AfdianTotalEarningsWidget::class.java).forEach { id ->
                AfdianTotalEarningsWidget().update(context, id)
            }
        }
    }
}

class RefreshAfdianTotalEarningsAction : ActionCallback {
    override suspend fun onAction(context: Context, glanceId: GlanceId, parameters: ActionParameters) {
        AfdianTotalEarningsWidget().update(context, glanceId)
    }
}

@Composable
private fun AfdianTotalEarningsContent(earnings: AfdianEarnings?, errorMsg: String?, isDark: Boolean, isConfigured: Boolean) {
    ...
    val amountText = if (!isConfigured) {
        ...
        formatAmount(earnings.totalAmount)
    ...
    Box(
        ...
        Box(
            modifier = GlanceModifier
                ...
                .background(bgColor)
                .padding(pad)
                .clickable(onClick = actionRunCallback<RefreshAfdianTotalEarningsAction>())
        ) {
...
class AfdianTotalEarningsWidgetReceiver : GlanceAppWidgetReceiver() {
    override val glanceAppWidget: GlanceAppWidget = AfdianTotalEarningsWidget()
}